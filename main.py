...
def seed_admin():
    ...
    x=db(); existing=x.execute("SELECT id FROM users WHERE username=?",(u,)).fetchone()
    if existing:
        x.execute("UPDATE users SET password_hash=?,role='admin',active=1,family_id=NULL WHERE id=?",(hp(p),existing["id"]))
    else:
        x.execute("INSERT INTO users(username,password_hash,role,created_at) VALUES(?,?,?,?)",(u,hp(p),"admin",now().isoformat()))
    x.commit(); x.close()
...
@app.get("/")
def root(): return {"name":"Ghar Parivar","status":"online","version":"2.3.0"}
@app.get("/health")
...
def login(d:Login,response:Response):
    ...
    t=secrets.token_urlsafe(48); x.execute("INSERT INTO sessions(token,user_id,expires_at) VALUES(?,?,?)",(t,u["id"],(now()+timedelta(hours=SESSION_HOURS)).isoformat())); x.commit(); x.close()
...
@app.get("/media")
def public_media():
    x=db(); r=x.execute("SELECT id,slot,image_data,caption FROM site_media WHERE active=1 ORDER BY id").fetchall(); x.close(); return [dict(a) for a in r]

@app.get("/family/private")
def private_family(u=Depends(user)):
    x=db()
    if u["role"]=="admin":
        r=x.execute("SELECT * FROM family_members WHERE active=1 ORDER BY family_id,id").fetchall(); fid="all"
    else:
        if not u["family_id"]: x.close(); raise HTTPException(404,"No family is assigned to this account")
        r=x.execute("SELECT * FROM family_members WHERE family_id=? AND active=1 ORDER BY id",(u["family_id"],)).fetchall(); fid=u["family_id"]
    x.close(); return {"family_id":fid,"members":[dict(a) for a in r]}
...
def create_user(d:NewUser,u=Depends(admin)):
    x=db()
    try:
        c=x.execute("INSERT INTO users(username,password_hash,role,family_id,created_at) VALUES(?,?,?,?,?)",(d.username.strip(),hp(d.password),"family",d.family_id.strip(),now().isoformat())); x.commit(); return {"message":"Family account created","user_id":c.lastrowid,"username":d.username,"family_id":d.family_id}
